...
void BuildSectionButtons(SectionBuilder &builder) {
	...
	// MZGram: right below the account, so fork features are one click away.
	builder.addSectionButton({
		.title = rpl::single(u"MZGram"_q),
		.targetSection = MZGramId(),
		.icon = { &st::menuIconStealth },
		.keywords = { u"mzgram"_q, u"ghost"_q, u"deleted"_q, u"edited"_q },
	});
